...
router.get('/', requireAuth, async (req, res) => {
  try {
    const { rows } = await sql.query('SELECT * FROM bookings WHERE user_id = $1 ORDER BY booked_at DESC', [req.user.id]);
    res.json(rows);
  } catch (err) {
    console.error(err);
...
router.put('/:id/cancel', requireAuth, async (req, res) => {
  try {
    await sql.query("UPDATE bookings SET status = 'cancelled' WHERE id = $1 AND user_id = $2", [req.params.id, req.user.id]);
